...
\begin{align*}
	...
	\end{bmatrix} \\
\end{align*}
This matrix has positive eigenvalues that are calculated using armadillos eig_sym-function. The test is passed if the three eigenvalues are the same.

There is also a function that creates a random symmetric matrix of size $(n\times n)$, and checks the jacobi-algortihm versus armadillos eig_sym-function.

\section{Conclusion and discussion}
\lststyle{txtstyle}
\lstinputlisting{../pythondata/noninteracting.dat}
%one electron
%TODO data from one electron
%rhoN dependency
%N dependency
%two electrons
